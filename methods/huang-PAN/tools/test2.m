close all
clear
I= load_images('F:\backup_f\images\3');
[R,C,~,N] = size(I);
...
figure,imshow([I_v(:,:,1) I_v(:,:,2) I_v(:,:,3) I_v(:,:,4) I_v(:,:,5)]),title('自�?应伽马增强亮�'),impixelinfo;
figure,imshow([I_v_lcc(:,:,1) I_v_lcc(:,:,2) I_v_lcc(:,:,3) I_v_lcc(:,:,4) I_v_lcc(:,:,5)]),title('自�?�?.0伽马增强亮度'),impixelinfo;
% figure,imshow([pca_t(:,:,1) pca_t(:,:,2) pca_t(:,:,3) pca_t(:,:,4) pca_t(:,:,5)]),title('pca�?亮度'),impixelinfo;
...
%     end
% end
figure,imshow([res res2 result]),title('propose1—�??propose2—�?edd');impixelinfo
value_test2(I,res2);
