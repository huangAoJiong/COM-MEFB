% clear;
close all
gray = imread('F:\backup_f\images\3\监视50m帧计数19362_曝光时间6.4ms_时标0s_register.tif');
% gray = rgb2gray(gray);
gray = im2double(gray);
% gray = res_temp(:,:,3);
gray_inv = 1 - gray;
...
lcc = 1 * ((gray / 1) .^ (2 .^((0.5 - m) / 0.5)));         


imshow([gray m lcc]);impixelinfo

%{

A1=imread('car.png');
w=str2double(inputdlg('璇疯緭鍏ユ护娉㈢獥鍙ｅぇ灏?, 'INPUT scale factor', 1, {'2'}));
z=(2*w+1)*(2*w+1);
%A2=imnoise(A1,'salt & pepper',0.01);
%A2=rgb2gray(A2);
[nrows,ncols,ncoms]=size(A1);
OUT = uint8(zeros(nrows,ncols,ncoms));%杈撳嚭鍥惧儚
if(ncoms==1)
    S=zeros(nrows,ncols);
    S(1,1)=A1(1,1);
    for u=2:nrows
...
imshow(A1),title('鍘熷浘');
figure;
%imshow(A1),title('鍔犲櫔澹板悗鐨勫浘鐗?);
%figure;
imshow(OUT),title('鍧囧?婊ゆ尝鍚庣殑鍥剧墖');
%}